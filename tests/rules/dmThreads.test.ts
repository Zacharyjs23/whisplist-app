...
describeMaybe('firestore rules - dmThreads', () => {
  ...
  test('participant can create message', async () => {
    ...
    await assertSucceeds(
      db
        .doc('dmThreads/thread1/messages/m1')
        .set({ senderId: 'user2', text: 'hi', timestamp: 1 }),
    );
  });
  ...
  test('non-participant cannot create message', async () => {
    const db = testEnv.authenticatedContext('user3').firestore();
    await assertFails(
      db.doc('dmThreads/thread1/messages/m2').set({ text: 'nope', ts: 2 }),
    );
  });
  ...
  test('participant can write own typing doc', async () => {
    const db = testEnv.authenticatedContext('user1').firestore();
    await assertSucceeds(
      db.doc('dmThreads/thread1/typing/user1').set({ typing: false }),
    );
  });
  ...
  test('typing doc rejects unexpected keys and non-boolean typing', async () => {
    ...
    await assertFails(
      db
        .doc('dmThreads/thread1/typing/user1')
        .set({ typing: false, extra: true }),
    );
  });
  ...
  test("participant cannot write another user's typing doc", async () => {
    const db = testEnv.authenticatedContext('user1').firestore();
    await assertFails(
      db.doc('dmThreads/thread1/typing/user2').set({ isTyping: true }),
    );
  });
  ...
  test('non-participant cannot write typing doc', async () => {
    const db = testEnv.authenticatedContext('user3').firestore();
    await assertFails(
      db.doc('dmThreads/thread1/typing/user3').set({ isTyping: true }),
    );
  });
  ...
  test('participant can update readReceipts but not change participants', async () => {
    ...
    await assertSucceeds(
      db.doc('dmThreads/thread1').update({ ['readReceipts.user1']: 123 }),
    );
    // Disallowed: attempt to change participants array
    await assertFails(
      db.doc('dmThreads/thread1').update({ participants: ['user1', 'user3'] }),
    );
  });
});
