...
describeMaybe('firestore rules - comments', () => {
  ...
  beforeAll(async () => {
    ...
    await testEnv.withSecurityRulesDisabled(async (context: any) => {
      ...
      await adminDb
        .doc('wishes/wishC/comments/c1')
        .set({ userId: 'user1', text: 'hello' });
      await adminDb
        .doc('wishes/wishC/comments/c2')
        .set({ userId: 'user1', text: 'world' });
    });
  });
  ...
  test('author can create comment <= 280', async () => {
    ...
    await assertSucceeds(
      db
        .doc('wishes/wishC/comments/new1')
        .set({ userId: 'user2', text: 'short text' }),
    );
  });
  ...
  test('author can create reply when parent exists', async () => {
    ...
    await assertSucceeds(
      db
        .doc('wishes/wishC/comments/newReply')
        .set({ userId: 'user2', text: 'reply', parentId: 'c1' }),
    );
  });
  ...
  test('cannot create reply if parent comment missing', async () => {
    ...
    await assertFails(
      db
        .doc('wishes/wishC/comments/missingParent')
        .set({ userId: 'user2', text: 'reply', parentId: 'does-not-exist' }),
    );
  });
  ...
  test('create with long text fails', async () => {
    ...
    await assertFails(
      db
        .doc('wishes/wishC/comments/new2')
        .set({ userId: 'user2', text: longText }),
    );
  });
  ...
  test('non-author cannot update comment', async () => {
    ...
    await assertFails(
      db.doc('wishes/wishC/comments/c1').update({ text: 'hacked' }),
    );
  });
  ...
  test('author can update comment within 280 chars', async () => {
    ...
    await assertSucceeds(
      db.doc('wishes/wishC/comments/c1').update({ text: 'edited' }),
    );
  });
  ...
  test('author cannot update comment beyond 280 chars', async () => {
    ...
    await assertFails(
      db.doc('wishes/wishC/comments/c2').update({ text: longText }),
    );
  });
  ...
  test('author cannot change parentId after creation', async () => {
    await testEnv.withSecurityRulesDisabled(async (context: any) => {
      await context
        .firestore()
        .doc('wishes/wishC/comments/childWithParent')
        .set({ userId: 'user1', text: 'child', parentId: 'c1' });
    });
    const db = testEnv.authenticatedContext('user1').firestore();
    await assertFails(
      db
  ...
  test('non-author cannot delete comment', async () => {
    ...
    await testEnv.withSecurityRulesDisabled(async (context: any) => {
      await context
        .firestore()
        .doc('wishes/wishC/comments/c3')
        .set({ userId: 'user1', text: 'ok' });
    });
    const db = testEnv.authenticatedContext('user2').firestore();
