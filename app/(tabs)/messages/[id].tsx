...
export default function MessageThread() {
  ...
  const starterSuggestions = React.useMemo(() => {
    const personalized = otherProfile?.displayName
      ? [
          `Hey ${otherProfile.displayName}, want to collaborate on this?`,
          ...STARTER_SUGGESTIONS,
        ]
      : STARTER_SUGGESTIONS;
  ...
  useEffect(() => {
    ...
    const unsub = listenTyping(String(id), (map) => {
      const others = Object.entries(map).filter(
        ([k, v]) => k !== user.uid && v.typing,
      );
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(ref, (snap) =>
      setOtherProfile((snap.data() as any) || {}),
    );
    return unsub;
  }, [otherUid]);
  ...
  const onSend = useCallback(async () => {
    ...
    try {
      ...
      setReplyTo(null);
      try {
        await setTyping(String(id), user.uid, false);
      } catch {}
    } finally {
      setUploading(false);
    }
        ...
        >
          <TouchableOpacity
            style={[
              styles.bubble,
              {
                backgroundColor: mine ? theme.tint : theme.input,
          ...
          >
            {item.replyToId && (
              <View
                style={{
                  borderLeftWidth: 2,
                  borderLeftColor: theme.placeholder,
                  paddingLeft: 6,
                  marginBottom: 6,
                }}
              >
                <Text style={{ color: theme.placeholder, fontSize: 12 }}>
                  ↩︎{' '}
                  {item.replyToText || (item.replyToImageUrl ? '[photo]' : '')}
                </Text>
              </View>
            )}
            {item.imageUrl ? (
    ...
    >
      ...
      {messages.length < 6 && starterSuggestions.length > 0 && (
        <View style={{ paddingHorizontal: 12, paddingBottom: 8 }}>
          <Text
            style={{ color: theme.placeholder, fontSize: 12, marginBottom: 6 }}
          >
            Ice breakers
          </Text>
          <View style={styles.starterRow}>
            {starterSuggestions.map((line) => (
              <TouchableOpacity
                key={line}
                onPress={() =>
                  setText((prev) => (prev ? `${prev} ${line}` : line))
                }
      ...
      <View style={[styles.inputBar, { backgroundColor: theme.input }]}>
        <TextInput
          ...
          onChangeText={(v) => {
            ...
            if (id && user?.uid) {
              try {
                void setTyping(String(id), user.uid, true);
              } catch {}
              if (typingTimer.current) clearTimeout(typingTimer.current);
              typingTimer.current = setTimeout(() => {
                try {
                  void setTyping(String(id!), user!.uid, false);
                } catch {}
              }, 1500);
            }
          }}
        ...
        <TouchableOpacity
          onPress={onSend}
          disabled={(!text.trim() && !pendingImageUri) || uploading}
        >
          <Text style={{ color: text.trim() ? theme.tint : theme.placeholder }}>
        ...
        <View style={{ paddingHorizontal: 12, paddingBottom: 8 }}>
          {replyTo && (
            ...
            >
              <Text style={{ color: theme.placeholder, fontSize: 12 }}>
                ↩︎{' '}
                {replyTo.text || ((replyTo as any).imageUrl ? '[photo]' : '')}
              </Text>
