...
export const getDeveloperMetrics = https.onCall(
  async (_data: unknown, context: any) => {
    ...
    try {
      ...
      const isDeveloper =
        userSnap.exists && userSnap.get('developerMode') === true;
      if (!isDeveloper) {
      ...
      const [wishCountSnap, boostCountSnap, giftCountSnap, recentUsersSnap] =
        await Promise.all([
      ...
      logger.error('Failed to load developer metrics', error);
