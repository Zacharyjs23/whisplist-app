...
export const SupporterPaywallModal: React.FC<Props> = ({
  visible,
  onClose,
  onSubscribe,
  perks,
      ...
      <View style={styles.backdrop}>
        <View style={[styles.card, { backgroundColor: theme.input }]}>
          ...
          <Text style={{ color: theme.placeholder, marginBottom: 10 }}>
            {t(
              'premium.rephrase.body',
              'Rephrase is available for supporters. Open subscriptions to join?',
            )}
          </Text>
          ...
          <View style={{ flexDirection: 'row', gap: 10 }}>
            <TouchableOpacity
              style={[styles.btn, { backgroundColor: theme.input }]}
              onPress={onClose}
            >
              <Text style={{ color: theme.text }}>
                {t('premium.rephrase.cancel', 'Not now')}
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
